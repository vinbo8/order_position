...
#
#
# This code was modified by ${USER} ${USER}
#
"""
...
class LineByLineTextDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str, block_size=512):
        ...
        self.examples = tokenizer(lines, add_special_tokens=True, truncation=True, max_length=block_size)["input_ids"]
...
def mask_tokens(inputs: torch.Tensor, tokenizer: PreTrainedTokenizer, args, model: PreTrainedModel) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    if args.compute_loss_on_all:
        # compute loss on all labels except padding
        ...
        for step, batch in enumerate(epoch_iterator):
            if args.adjust_replacement_probs_with_time > 0:
                if args.replacement_probs[1] - args.adjust_replacement_probs_with_time >= 0.0:
                    # shift some probability to nn replacement
                    args.replacement_probs[1] = args.replacement_probs[1] - args.adjust_replacement_probs_with_time
                    args.replacement_probs[2] = args.replacement_probs[2] + args.adjust_replacement_probs_with_time
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.lca_steps != -1:
                    for k, v in model.named_parameters():
                        ...
                        # log these 10 times during training
                        if (step % args.num_train_epochs) == 0:
                            opath = os.path.join(args.output_dir, '_lca_logs.json')
...
def evaluate(args, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, prefix="", section="full") -> Dict:
    ...
    result = {f"{section}_perplexity": perplexity}
...
def main():
    ...
    parser.add_argument("--lca_steps", type=int, default=-1)
    ...
    parser.add_argument('--position_embedding_type', action='store', default='absolute')    
    parser.add_argument('--ffn_type', action='store', default='linear')    

    parser.add_argument("--crosslingual_context", type=float, default=-1, help="if > 0 replace a proportion of non-masked context words with nearest neighbours")
    ...
    parser.add_argument("--compute_loss_on_all", action="store_true", help="")
    parser.add_argument("--use_anchor_points", action="store_true", help="")
    parser.add_argument("--adjust_replacement_probs_with_time", type=float, default=-1, help="if > 0 move from random to nn replacement")
    ...
    logger.info(f"Position embedding: {config.position_embedding_type}")
    logger.info(f"Position embedding: {config.ffn_type}")
    ...
    logger.info(f"special -- {args.special_token_indices}")
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(args, model, tokenizer, prefix=prefix, section="full")
            result = dict((k + "_{}".format(global_step), v) for k, v in result.items())
            results.update(result)
            result = evaluate(args, model, tokenizer, prefix=prefix, section="half")
